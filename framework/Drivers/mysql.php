...
class db_mysql {
	...
	public function __construct($dbConfig)
	{
		...
		$this->exec("SET NAMES ".spConfig('db.charset'));
        */
	}
    ...
    public function connect(){
		...
		$linkfunction = ( TRUE == $dbConfig['persistent'] ) ? 'mysql_pconnect' : 'mysql_connect';
		$this->conn = $linkfunction($dbConfig['host'].":".$dbConfig['port'], $dbConfig['login'], $dbConfig['password'], true) or spError("数据库链接错误 : " . mysql_error()); 
		mysql_select_db($dbConfig['database'], $this->conn) or spError("无法找到数据库，请确认数据库名称正确！");
		$this->exec("SET NAMES ".spConfig('db.charset'));
    }
	...
	{
		if( TRUE != spConfig('db.persistent') )@mysql_close($this->conn);
	}
}
