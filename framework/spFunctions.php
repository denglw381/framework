...
function spRun(){
	...
	// 控制器程序运行完毕，进行模板的自动输出
	if(FALSE != spConfig('view.auto_display')){
		$__tplname = $__controller.spConfig('view.auto_display_sep').
			$__action.spConfig('view.auto_display_suffix'); // 拼装模板路径
...
function dump($vars, $output = TRUE, $show_trace = FALSE){
	...
	if(TRUE != SP_DEBUG && TRUE != spConfig('allow_trace_onrelease'))return;
...
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE){
	if(spConfig("import_file.".md5($sfilename))) return TRUE; // 已包含载入，返回
	// 检查$sfilename是否直接可读
	if( TRUE == @is_readable($sfilename) ){
		require($sfilename); // 载入文件
		spConfig('import_file.'.md5($sfilename), TRUE); // 对该文件进行标识为已载入
		return TRUE;
	}else{
		if(TRUE == $auto_search){ // 需要搜索文件
			// 按“应用程序包含目录->应用程序Model目录->sp框架包含文件目录”的顺序搜索文件
			foreach(array_merge(spConfig('include_path'), array(spConfig('model_path')), spConfig('sp_include_path')) as $sp_include_path){
				// 检查当前搜索路径中，该文件是否已经载入
				if("" == spConfig("import_file".md5($sp_include_path.'/'.$sfilename))) return TRUE;
				if( is_readable( $sp_include_path.'/'.$sfilename ) ){
					require($sp_include_path.'/'.$sfilename);// 载入文件
					spConfig('import_file'.md5($sp_include_path.'/'.$sfilename), TRUE);// 对该文件进行标识为已载入
					return TRUE;
				}
			}
		}
...
function spAccess($method, $name, $value = NULL, $life_time = -1){
	...
	if(!is_dir(spConfig('sp_cache'))) __mkdirs(spConfig('sp_cache'));
	$sfile = spConfig('sp_cache').'/'.spConfig('sp_app_id').md5($name).".php";
...
function spClass($class_name, $args = null, $sdir = null, $force_inst = FALSE){
	...
	if(TRUE != $force_inst) if(spConfig("inst_class.".$class_name) !="") return spConfig("inst_class.".$class_name);

	// 如果$sdir不能读取，则测试是否仅路径
	if(null != $sdir && !import($sdir) && !import($filename =$sdir.'/'.$class_name.'.php'))return FALSE;
	$has_define = FALSE;
	// 检查类定义是否存在
	...
	if(FALSE != $has_define){
		...
		eval("spConfig('inst_class.'.\$class_name, new \$class_name($argString));"); 
		return spConfig("inst_class.".$class_name);
	}
...
function spError($msg, $output = TRUE, $stop = TRUE){
	if(spConfig('sp_error_throw_exception')) throw new Exception($msg);
	if(TRUE != SP_DEBUG){log::write($msg);if(TRUE == $stop)exit;}
	$traces = debug_backtrace();
	$bufferabove = ob_get_clean();
	require_once(spConfig('sp_notice_php'));
	if(TRUE == $stop) exit;
}
 ...
 */
function T($w) {
	$method = spConfig("lang.".spController::getLang());
...
function spUrl($controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE) {
    ...
    if(count($args) > 2){
            return spUrl_2($controller, $action, $args, $anchor);
    }
	if(TRUE == spConfig('htmli.enabled') && TRUE != $no_sphtml){
	...
	if( TRUE == spConfig('url.url_path_info')){ // 使用path_info方式
		...
		if(false !== strpos($controller,'/')){
			...
			$module = $arr[0];
			$controller = $arr[1];
			$url .= spConfig('url_module').'='.$module.'&'; 
		}
		$url .= spConfig("url_controller")."={$controller}&";
		$url .= spConfig("url_action")."={$action}";
		if(null != $args)foreach($args as $key => $arg) $url .= "&{$key}={$arg}";
	}
 ...
 * @param mode    文件权限
 */
function spMkdirs($dir, $mode = 0777)
{
	if (!is_dir($dir)) {
		spMkdirs(dirname($dir), $mode);
...
{
	return spConfig('ext.'.$ext_node_name) == "" ? FALSE : spConfig('ext.'.$ext_node_name);
}
...
{
	return spConfig("view_registered_functions.".$alias,$callback_function);
}
...
function spDB($tbl_name, $pk = null){
	...
	$modelObj->tbl_name = (TRUE == spConfig("db_spdb_full_tblname")) ? $tbl_name :	spConfig('db.prefix') . $tbl_name;
 ...
 * 配置文件读取函数
 */
function spConfig($name="",$value=""){
        $argc = func_num_args();
        static $_config = [], $config_file_cache = [];
        if(is_array($name)){ //设置配置数据
                ...
                if($argc==1){
                        $keys = explode(".",$name); 
                        $file = realpath(__dir__.'/../config/'.$keys[0].'.php');
                        if(!isset($config_file_cache[$file]) && file_exists($file)){
                                $configs = include($file);
                                spConfig($keys[0], $configs);
                                $config_file_cache[$file] = TRUE;
                        }
                        if(strpos($name,".")){
...
//加载model类
function model($name, $path = NULL){
        return spClass($name.'Model',NULL,spConfig('model_path').$name.'Model.php');
}
//加载service类
function service($name, $args = null, $path = NULL){
        return spClass($name.'Service', $args, spConfig('service_path').$name.'Service.php');
}

function plugin($name, $args = NULL){
        return spClass($name.'Plugin', $args, spConfig('plugin_path').$name.'Plugin.php');
}

//加载action类
function action($name, $path = NULL){
}

//加载lib库
function library($name, $args = NULL){
        return spClass($name, $args, spConfig('library_path').$name.'.class.php');
}
//加载工具类
function utile($name, $args = NULL){
        return spClass($name, $args, spConfig('utile_path').$name.'.class.php');
}
...
function spThumb($filePath = '', $width = 100, $height = 100){
        if($filePath === ''){
                return false;
        }
        return '/thumb.php?src='.$filePath.'&w='.$width.'&h='.$height;
        return $filePath.'_thumb_'.$width.'_'.$height.'.'.$ext; //rewite模式链接
}
...
function spErrorHandler($errno, $errstr, $errfile, $errline)
{
        ...
        switch ($errno) {
                case E_USER_ERROR:
                        spLog::error($errstr." ".$errfile." ".$errline);
                        break;

                case E_USER_WARNING:
                        spLog::warn($errstr." ".$errfile." ".$errline);
                        break;

                case E_USER_NOTICE:
                        spLog::notice($errstr." ".$errfile." ".$errline);
                        break;
...
//业务公共函数库
function read_field_from_list($field, $list){
        $arr = array();
        foreach($list as $val){
                $arr[] = $val[$field];
        }   
...
function read_key_value_from_list($key, $value, $list){
        $arr = array();
        foreach($list as $val){
...
function __log(){
        $vars = func_get_args();
        error_log(json_encode($vars), 3, '/tmp/denglw.log');
        SpLog::debug($vars);
}
 ...
 * @param no_sphtml    是否应用spHtml设置，在FALSE时效果与不启用spHtml相同。
 */
function spUrl_2($controller = null, $args = null, $anchor = null, $no_sphtml = FALSE) {
        if(false === strpos($controller,'/')) return '#';
        else{
                $arr = explode('/',$controller);
                if(count($arr) == 2){
                        $controller = $arr[0];
...
#剩余时间代码
function friendlyLeftTime($time){
        $retStr = '';
        $day = floor($time/(24*3600));
        $hour = floor(($time - $day*24*3600)/3600);
        $minute = floor(($time - ($day*24 + $hour)*3600)/60);
        $second = floor($time % 60);
        if($day) $retStr .=$day.'天';
        if($hour) $retStr .=$hour.'小时';
        if($minute) $retStr .=$minute.'分';
        if($second) $retStr .= $second . '秒';
        return $retStr;
}
 ...
 *记录请求数据
 */
function spRecordRequest($info, $record = false){
        static $logs = [];
        if($info) $logs[] = $info;
        if($record){
            SpLog::request($logs);       
 ...
 *记录日志
 */
function spRecordLog(){
       spRecordRequest(null, true);
       SpLog::save();
}
