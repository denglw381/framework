...
//
ob_start();
if (!include_once(LIB_THUMB_PATH.'/phpthumb.functions.php')) {
	ob_end_flush();
	die('failed to include_once("'.realpath(LIB_THUMB_PATH.'/phpthumb.functions.php').'")');
}
ob_end_clean();
class phpthumb {
...
//		}
	...
	function RenderOutput() {
		...
		switch ($this->thumbnailFormat) {
			...
			case 'bmp':
				...
				if (!@include_once(LIB_THUMB_PATH.'/phpthumb.bmp.php')) {
					$this->DebugMessage('Error including "'.LIB_THUMB_PATH.'/phpthumb.bmp.php" which is required for BMP format output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
			...
			case 'ico':
				...
				if (!@include_once(LIB_THUMB_PATH.'/phpthumb.ico.php')) {
					$this->DebugMessage('Error including "'.LIB_THUMB_PATH.'/phpthumb.ico.php" which is required for ICO format output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
	...
	function setOutputFormat() {
		...
		if (@is_readable(LIB_THUMB_PATH.'/phpthumb.ico.php')) {
			$AvailableImageOutputFormats[] = 'ico';
		}
		if (@is_readable(LIB_THUMB_PATH.'/phpthumb.bmp.php')) {
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (preg_match('#^[a-z0-9]+\:/{1,2}#i', $filename)) {
			...
			$AbsoluteFilename = LIB_THUMB_WEB_ROOT.DIRECTORY_SEPARATOR.preg_replace('#[/\\\\]#', DIRECTORY_SEPARATOR, $filename);
		...
		if (!$this->config_allow_src_above_phpthumb && !preg_match('#^'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', LIB_THUMB_PATH)).'#', $AbsoluteFilename)) {
			$this->DebugMessage('!$this->config_allow_src_above_phpthumb therefore setting "'.$AbsoluteFilename.'" (outside "'.LIB_THUMB_PATH.'") to null', __FILE__, __LINE__);
			return false;
		}
...
// newer versions of ImageMagick require -repage <geometry>
	...
	function Rotate() {
		if ($this->ra || $this->ar) {
			...
			if (!include_once(LIB_THUMB_PATH.'/phpthumb.filters.php')) {
				$this->DebugMessage('Error including "'.LIB_THUMB_PATH.'/phpthumb.filters.php" which is required for applying filters ('.implode(';', $this->fltr).')', __FILE__, __LINE__);
				return false;
			}
...
exit;
		...
		if ($erase) {
			...
			if (is_file($this->config_nooffsitelink_watermark_src)) {
				...
				if (!include_once(LIB_THUMB_PATH.'/phpthumb.filters.php')) {
					$this->DebugMessage('Error including "'.LIB_THUMB_PATH.'/phpthumb.filters.php" which is required for applying watermark', __FILE__, __LINE__);
					return false;
				}
			...
			foreach ($this->fltr as $filtercommand) {
				...
				switch ($command) {
					...
					case 'usm': // UnSharpMask sharpening
						...
						if (phpthumb_functions::gd_version() >= 2.0) {
							ob_start();
							if (!@include_once(LIB_THUMB_PATH.'/phpthumb.unsharp.php')) {
								$include_error = ob_get_contents();
								if ($include_error) {
									$this->DebugMessage('include_once("'.LIB_THUMB_PATH.'/phpthumb.unsharp.php") generated message: "'.$include_error.'"', __FILE__, __LINE__);
								}
								$this->DebugMessage('Error including "'.LIB_THUMB_PATH.'/phpthumb.unsharp.php" which is required for unsharp masking', __FILE__, __LINE__);
								ob_end_clean();
								return false;
							}
							ob_end_clean();
		...
		if (!$this->gdimg_source) {
			...
			switch (@$this->getimagesizeinfo[2]) {
				case 6:
					...
					if (!@include_once(LIB_THUMB_PATH.'/phpthumb.bmp.php')) {
						ob_end_clean();
						return $this->ErrorImage('include_once('.LIB_THUMB_PATH.'/phpthumb.bmp.php) failed');
					}
					ob_end_clean();
		...
		if ($tempnam = $this->phpThumb_tempnam()) {
			if ($fp_tempnam = @fopen($tempnam, 'wb')) {
				...
				if (($ICFSreplacementFunctionName == 'ImageCreateFromGIF') && !function_exists($ICFSreplacementFunctionName)) {
					...
					ob_start();
					if (!@include_once(LIB_THUMB_PATH.'/phpthumb.gif.php')) {
						$ErrorMessage = 'Failed to include required file "'.LIB_THUMB_PATH.'/phpthumb.gif.php" in '.__FILE__.' on line '.__LINE__;
