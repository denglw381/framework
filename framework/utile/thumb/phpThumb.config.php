...
ob_start();
if (!file_exists(LIB_THUMB_PATH.'/phpthumb.functions.php') || !include_once(LIB_THUMB_PATH.'/phpthumb.functions.php')) {
	ob_end_flush();
	die('failed to include_once(phpthumb.functions.php) - realpath="'.realpath(LIB_THUMB_PATH.'/phpthumb.functions.php').'"');
}
ob_end_clean();
...
$PHPTHUMB_CONFIG['document_root'] = realpath((getenv('DOCUMENT_ROOT') && preg_match('#^'.preg_quote(realpath(getenv('DOCUMENT_ROOT'))).'#', realpath(__FILE__))) ? getenv('DOCUMENT_ROOT') : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace(DIRECTORY_SEPARATOR, '/', LIB_THUMB_PATH)));
...
$PHPTHUMB_CONFIG['cache_directory'] = LIB_THUMB_CACHE_PATH;                            // set the cache directory relative to the phpThumb() installation
...
//	$PHPTHUMB_CONFIG['cache_directory'] = LIB_THUMB_PATH.'/cache/'; // set the cache directory to an absolute directory for all source images
//} else {
...
$PHPTHUMB_CONFIG['cache_source_directory'] = LIB_THUMB_PATH.'/cache/source/';  // set the cache directory for unprocessed source images
...
$PHPTHUMB_CONFIG['ttf_directory'] = LIB_THUMB_PATH.'/fonts'; // Base directory for TTF font files
...
function phpThumbURL($ParameterString) {
	...
	return str_replace(@$PHPTHUMB_CONFIG['document_root'], '', LIB_THUMB_PATH).DIRECTORY_SEPARATOR.'phpThumb.php?'.$ParameterString.'&hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']);
}
