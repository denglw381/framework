...
class spView {
	...
	{
		if(FALSE == spConfig('view.enabled'))return FALSE;
		if(FALSE != spConfig('view.auto_ob_start'))ob_start();
		$engine = spConfig('view.engine_name');
		$this->$engine();

		if( ""!=(spConfig('sp_app_id')) && isset($this->engine->compile_id) )$this->engine->compile_id = spConfig('sp_app_id');
		// 检查编译目录是否可写
		if( empty($this->engine->no_compile_dir) && (!is_dir($this->engine->compile_dir) || !is_writable($this->engine->compile_dir))) spMkdirs($this->engine->compile_dir);
	...
	private function smarty(){
		$this->engine = spClass(spConfig('view.engine_name'),null,spConfig('view.engine_path'));
		if( spConfig('view.config') && is_array(spConfig('view.config'))){
			$engine_vars = get_class_vars(get_class($this->engine));
			//var_dump($engine_vars);
			foreach( spConfig('view.config') as $key => $value ){
				if( array_key_exists($key,$engine_vars) )$this->engine->{$key} = $value;
			}
			$config = spConfig('view.config');
			$this->engine->setTemplateDir($config['template_dir'])
				->setCompileDir($config['compile_dir'])
				->setPluginsDir(SMARTY_PLUGINS_DIR)
				->setCacheDir($config['cache_dir'])
				->setConfigDir(SP_PATH. DS . 'configs' . DS);
			if(spConfig('view.cache')){
	...
	#变量赋值
	function assign($data, $val){
		$this->engine->assign($data, $val);
	}
	...
	public function display($tplname)
	{
		try {
				$this->addfuncs();
				$this->displayed = TRUE;
				if(spConfig('view.debugging') && SP_DEBUG)$this->engine->debugging = TRUE;
				$this->engine->display($tplname);
		} catch (Exception $e) {
			spError( spConfig('view.engine_name'). ' Error: '.$e->getMessage() );
		}
	}
	...
	{
		if( is_array(spConfig("view_registered_functions")) ){
			foreach( spConfig("view_registered_functions") as $alias => $func ){
				if( is_array($func) && !is_object($func[0]) )$func = array(spClass($func[0]),$func[1]);
				$this->engine->registerPlugin("function", $alias, $func);
				spConfig("view_registered_functions.".$alias,"");
			}
		}
	}
	/**
	...
	{
		$controller = spConfig("default_controller");
		$action = spConfig("default_action");
		$args = array();
		$anchor = null;
		foreach($params as $key => $param){
			if( $key == spConfig("url_controller") ){
				$controller = $param;
			}elseif( $key == spConfig("url_action") ){
				$action = $param;
			}elseif( $key == 'anchor' ){
				$anchor = $param;
			}else{
...
class spHtml
{
	...
	public function make($spurl, $alias_url = null, $update_mode = 2)
	{
		...
		if( $url_item = spHtml::getUrl($controller, $action, $args, $anchor, TRUE) ){
			...
			$file_root_name = ( '' == spConfig('html.file_root_name')) ? 
									'' : spConfig('html.file_root_name').'/';
			if( null == $alias_url ){
			...
			$baseuri = rtrim(dirname(spConfig('url.url_path_base')), '/\\')."/".$filedir.$filename;
		...
		if( $url_list = spAccess('r', 'sp_url_list') ){
			...
			foreach( $url_list as $url ){
				if( substr($url,0,strlen($url_input)) == $url_input ){
					...
					if( TRUE == spConfig('html.safe_check_file_exists') && TRUE != $force_no_check ){
