...
class spModel {
	...
	{
		if( null == $this->tbl_name )$this->tbl_name = spConfig('db.prefix') . $this->table;
		if( '' == spConfig('db_driver_path')){
			spConfig('db_driver_path', spConfig('sp_drivers_path').'/'.spConfig('db.driver').'.php');
		}
		$this->_db = spClass('db_'.spConfig('db.driver'), array(0=>spConfig('db')),spConfig('db_driver_path'));
		$this->_initialize();
	}
   ...
   function commit(){
	...
	{
		if(in_array(strtolower($name),array('field','table','where','join','order','limit','page','having','group','lock','distinct'),true)) {
		...
		}elseif(in_array($name, spConfig("auto_load_model"))){
	...
	public function replace($conditions, $row)
	{
		if( $this->find($conditions) ){
			...
			$rows = array_merge($conditions, $row);
...
class spLinker
{
	...
	public function __call($func_name, $func_args){
		if( in_array( $func_name, $this->methods ) && FALSE != $this->enabled ){
			...
			return $run_result;
		}elseif(in_array($func_name, spConfig("auto_load_model"))){
