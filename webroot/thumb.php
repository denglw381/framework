...
//                                                         ///
//////////////////////////////////////////////////////////////
define('LIB_THUMB_WEB_ROOT', dirname(__FILE__));
define('LIB_THUMB_PATH', dirname(__FILE__).'/../framework/utile/thumb');
#define('LIB_THUMB_PATH', dirname(__FILE__));
define('LIB_THUMB_CACHE_PATH', dirname(__FILE__).'/../_runtime/img_cache');
...
ob_start();
if (!include_once(LIB_THUMB_PATH.'/phpthumb.class.php')) {
	ob_end_flush();
	die('failed to include_once("'.realpath(LIB_THUMB_PATH.'/phpthumb.class.php').'")');
}
ob_end_clean();
...
if (file_exists(LIB_THUMB_PATH.'/phpThumb.config.php')) {
	ob_start();
	if (include_once(LIB_THUMB_PATH.'/phpThumb.config.php')) {
		// great
	} else {
		ob_end_flush();
		$phpThumb->config_disable_debug = false; // otherwise error message won't print
		$phpThumb->ErrorImage('failed to include_once('.LIB_THUMB_PATH.'/phpThumb.config.php) - realpath="'.realpath(LIB_THUMB_PATH.'/phpThumb.config.php').'"');
	}
	ob_end_clean();
} elseif (file_exists(LIB_THUMB_PATH.'/phpThumb.config.php.default')) {
	$phpThumb->config_disable_debug = false; // otherwise error message won't print
...
$allowedGETparameters = array('src', 'w', 'h');
